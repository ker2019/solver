...
u = np.array([gmsh.view.probe(solution, x, 0, 0, distanceMax=-1, gradient=False)[0] for x in X])
ax1 = fig.add_subplot(2, 1, 1)
ax1.plot(X, u, label='Numerical solution')
ax1.plot(X, (1 - R/X)/(1 - R/L), label='Analytical solution')
ax1.legend()

dudx = np.array([gmsh.view.probe(solution, x, 0, 0, distanceMax=-1, gradient=True)[0][0] for x in X])
ax2 = fig.add_subplot(2, 1, 2)
ax2.plot(X, dudx, label='Numerical solution')
ax2.plot(X, (X**2 * (1/R - 1/L))**-1, label='Analytical solution')
ax2.legend()
